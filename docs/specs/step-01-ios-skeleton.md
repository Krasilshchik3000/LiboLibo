...
| **1.8** | **Никаких многоточий + download-икона в Фиде + чистый плеер + очистка истории:** убраны все `lineLimit` (тексты везде целиком), бесполезный play.circle.fill в строке Фида заменён на DownloadButton (cloud / progress / checkmark), плеер без `GeometryReader` — прогресс-слайдер не разъезжается, в «История» добавлена кнопка «Очистить» с подтверждающим алертом | ✅ [session 11](../sessions/2026-04-25-11-step-1.8.md) |
| **1.9** | **Плеер по Apple Podcasts:** обложка ограничена 300×300 (не разбухает), фиксированные `Spacer().frame(height:)` между блоками, `fixedSize(horizontal: false, vertical: true)` на заголовках (не убегают за края), системный Slider заменён на кастомный `CustomProgressBar` (серый трек + красный fill + белый thumb) — гарантированно видим на blurred backdrop | ✅ [session 12](../sessions/2026-04-25-12-step-1.9.md) |
